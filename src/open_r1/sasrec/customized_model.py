...
class SASRecModel(nn.Module):
    ...
    def recommend_topk(
        self,
        input_ids_item,             # [B, L] item sequence (0 = padding)
        k: int = 30,
        exclude_seen: bool = True,  # exclude items already in the sequence
        use_cosine: bool = False,   # use cosine similarity instead of dot product
        valid_mask: torch.Tensor = None,  # [N] bool; optional candidate-item mask
    ):
        """
        returns:
            topk_ids:   LongTensor [B, k]
            topk_scores: FloatTensor [B, k]
        """
        self.eval()

        # 1) Encode sequence: seq_out shape = [B, d]
        _, seq_out = self.forward(
            test_neg=None, answer_id=None,
            input_ids_item=input_ids_item,
            pos_ids_item=None, neg_ids_item=None
        )  # returns the last-timestep representation

        # 2) Item embedding matrix: [N, d]
        #   (attribute name may vary: .weight, .item_embeddings.weight, etc.)
        item_table = self.item_embedding.item_embeddings.weight  # [N, d]

        # 3) Similarity (dot product or cosine)
        if use_cosine:
            seq = F.normalize(seq_out, dim=-1)                  # [B, d]
            items = F.normalize(item_table, dim=-1)             # [N, d]
            logits = torch.matmul(seq, items.t())               # [B, N]
        else:
            logits = torch.matmul(seq_out, item_table.t())      # [B, N]

        # 4) (optional) Apply valid-item mask: invalid positions -> -inf
        if valid_mask is not None:
            # valid_mask: [N] bool, True = allowed
            neg_inf = torch.finfo(logits.dtype).min
            logits[:, ~valid_mask] = neg_inf

        # 5) (optional) Exclude previously interacted items
        if exclude_seen:
            # Block item ids that appear in each sequence
            # - skip padding id (e.g., 0)
            neg_inf = torch.finfo(logits.dtype).min
            B = input_ids_item.size(0)
            for b in range(B):
                seen = input_ids_item[b]
                seen = seen[seen > 0]            # drop padding
                if seen.numel() > 0:
                    logits[b, seen.unique()] = neg_inf

        # 6) Select top-k
        topk_scores, topk_ids = torch.topk(logits, k, dim=-1)   # [B, k], [B, k]
...
class ItemEncoder: # averages sub-token embeddings for each sequence item
    """
...
class DataArguments:
    ...
    data_name: str = field(default=None, metadata={"help": "Dataset name (e.g., amazon)."})
...
def sequential_loss(pooled_output, labels_id, neg_sample_id, labels_token_id, loss_type, training_args, num_virtual_tokens, tokenizer, item_encoder, item_enc_type, backbone_model):
    if loss_type == 'bpr':
        if training_args.peft_method=='p_tuning':
            ...
            # tensor = torch.full((labels_id.size(0), num_virtual_tokens), tokenizer.pad_token_id).to(labels_id.device)
            labels_id = torch.cat([tensor, labels_id], dim = 1) # [B, 16, subtoken] | [B, S, subtoken] 
...
def kl_divergence_loss(student_output, teacher_output, temperature):
    """
    student_output: attention-layer output of the student model
    teacher_output: attention-layer output of the teacher model
...
class AdaptiveLossWeighting(nn.Module):
    def __init__(self, num_losses):
        ...
        # Learnable log-variance per loss
        self.log_vars = nn.Parameter(torch.zeros(num_losses))

    def forward(self, losses):
        weighted_losses = 0
        for i, loss in enumerate(losses):
            # Weight each loss by its learned log-variance
...
def normalize_loss(loss, mean, std):
    return (loss - mean) / (std + 1e-6)  # avoid division by zero
