...
from customized_model import SASRecModel
import json
...
def train():
    ...
    sasrec_model = SASRecModel(model_args, data_args, training_args)
    ...
    betas = (0.9, 0.999)
    optim = torch.optim.Adam(sasrec_model.parameters(), lr=0.0001, betas=betas, weight_decay=0)
    trainer = Trainer(model=sasrec_model, tokenizer=None, args=training_args, **data_module)
        ...
        with torch.no_grad():
            for i, batch in rec_data_iter:
                ...
                if input_ids_item.shape[0]==batch_size:
                    test_logits_llm = sasrec_model.evaluate(answer_id=answer_id.cuda(), test_neg=test_neg.cuda(), input_ids_item=input_ids_item.cuda())
