...
def filter_valid_asins(string_list):
    ...
    Returns:
        list of str: only strings that start with 'abc' followed by digits
    """
...
def extract_items(text: str):
    """
    Extract <item_id>abc<digits></item_id> from anywhere in the string.
    Works even with newlines/whitespace around the tags.
    """
    try:
        items = re.findall(r"<item_id>\s*(abc\d+)\s*</item_id>", text, flags=0)[0]
    except:
        items = []
...
def inference():
    ...
    # model_name = f"model/google_gemma-3-4b-it_sdpo_001_amazon_Musical_Instruments_lora" #DPO
    # model_name = f"model/google_gemma-3-4b-it_sdpo_001_amazon_Musical_Instruments_lora" #DPO
    model_name = f"model/google_gemma-3-4b-it_grpo_001_amazon_Amazon_Fashion_lora" #DPO
