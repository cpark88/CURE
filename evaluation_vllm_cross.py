...
def inference():
    ...
    parser.add_argument("--source_domain", default='Amazon_Fashion', type=str)
    parser.add_argument("--target_domain", default='Clothing_Shoes_and_Jewelry', type=str)
    ...
    # model_name = f"model/google_gemma-3-4b-it_{stage}_{type_}_amazon_cdsr_lora"
    # model_name = f"model/google_gemma-3-4b-it_{stage}_{type_}_amazon_cdsr_only_single_lora"
    # model_name = f"model/google_gemma-3-4b-it_{stage}_{type_}_amazon_cdsr_grouped_reward_lora"
    ...
    model_name = f"model/google_gemma-3-4b-it_cadpo_001_amazon_{source_domain}_lora" #capo
    # model_name = f"model/google_gemma-3-4b-it_sdpo_001_amazon_{source_domain}_lora" #sdpo
    # model_name = f"model/google_gemma-3-4b-it_dpo_001_amazon_{source_domain}_lora" #dpo
    # model_name = f"model/google_gemma-3-4b-it_grpo_001_amazon_{source_domain}_lora" #grpo
    # model_name = f"model/google_gemma-3-4b-it_sft_001_amazon_{source_domain}_lora" #sft
    ...
    with open(f'data/cross_dataset/amazon_{source_domain}_{target_domain}_llm_test_20250521.json', 'r', encoding='utf-8') as f:
    ...
    rationals_name_ = f"""result/inference_list_{source_domain}_{target_domain}_test_{stage}_ndcg_{type_}_{task}.jsonl"""  
    ...
    # save
    rationals_name = f"""result/inference_list_{source_domain}_{target_domain}_test_{stage}_ndcg_{type_}_{task}.json"""
    with open(rationals_name, "w") as f:
        json.dump(rationals, f)  # JSON

        
    print("="*100)
    print(source_domain,'+',target_domain, 'with ',stage)
    k_=5    
